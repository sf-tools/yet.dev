...
export function goalUsageSummary(goal: ThreadGoal) {
...
export function createThreadGoal(objective: string, tokenBudget?: number): ThreadGoal {
  const now = Date.now();
  return {
...
export function isGoalUnfinished(goal: ThreadGoal) {
  return !['complete', 'budget_limited'].includes(goal.status);
}
