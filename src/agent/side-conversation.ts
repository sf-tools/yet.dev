...
function cloneFileChanges(state: AgentState) {
  return state.sessionFileChanges.map(change => ({
    ...change,
    stats: { ...change.stats },
  }));
}

export function cloneInactiveAgentState(state: AgentState): AgentState {
  const clone = hydratePersistedState(persistedStateFromAgentState(state, true));
  clone.sessionFileChanges = cloneFileChanges(state);
  return clone;
}
